...
# ======================================================
# 1. MNIST-Variation Loader
# ======================================================
class MNISTVariationData(object):
    def __init__(self):
        (trainX, trainY), (testX, testY) = keras_mnist.load_data()
        self.trainX = trainX.reshape(-1, 784).astype(np.float32) / 255.
        self.testX = testX.reshape(-1, 784).astype(np.float32) / 255.
        self.trainY = trainY
        self.testY = testY

        val_size = 5000
        self.valX = self.trainX[:val_size]
        self.valY = self.trainY[:val_size]
        self.trainX = self.trainX[val_size:]
        self.trainY = self.trainY[val_size:]

    def _one_hot(self, y, num_classes=10):
        return np.eye(num_classes)[y]

    def get_task_data(self, task_id):
        def apply_variation(X, t_id):
            X_mod = np.copy(X)
            if t_id == 1:
                pass
            elif t_id == 2:
                mask = X_mod < 0.1
                X_mod[mask] = np.random.rand(np.sum(mask))
            elif t_id == 3:
                X_mod = 1.0 - X_mod
        ...
        tr_X = apply_variation(self.trainX, task_id)
        v_X = apply_variation(self.valX, task_id)
        te_X = apply_variation(self.testX, task_id)

        tr_Y = self._one_hot(self.trainY)
        v_Y = self._one_hot(self.valY)
        te_Y = self._one_hot(self.testY)
...
# ======================================================
# 4. Generate 3 Tasks for MNIST-Variation
# ======================================================
dataset = MNISTVariationData()
num_tasks = 3
