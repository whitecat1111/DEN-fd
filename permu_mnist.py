...
class MNISTData(object):
    ...
    def __init__(self, one_hot=True):
        ...
        if one_hot:
            trainY = self._one_hot(trainY)
            valY = self._one_hot(valY)
            testY = self._one_hot(testY)

        self.train = self.DataSet(trainX, trainY)
        self.validation = self.DataSet(valX, valY)
        self.test = self.DataSet(testX, testY)
...
flags.DEFINE_string("dims", "784,312,128,10", "")
...
def get_den_capacity(params):
    return sum(np.prod(p.shape) for p in params.values())

def get_dnn_capacity(dims):
    cap = 0
    for i in range(1, len(dims)):
        cap += dims[i-1] * dims[i] + dims[i] # W + b
    return cap
    ...
    def __init__(self, dims, lr):
        ...
        with self.graph.as_default():
            ...
            for i in range(1, len(dims)):
                ...
                W = tf.Variable(
                    tf.random_normal([dims[i-1], dims[i]], stddev=0.01)
                )
...
task_permutation = [
    np.random.permutation(784) for _ in range(FLAGS.n_classes)
]

trainXs, valXs, testXs = [], [], []

for t in range(5):
    trainXs.append(trainX[:, task_permutation[t]])
    valXs.append(valX[:, task_permutation[t]])
    testXs.append(testX[:, task_permutation[t]])

# ======================================================
# ===== DEN TRAINING (Original) =====
# ======================================================
...
den_expansion_history = []
den_split_history = []

for t in range(5):

    data = (
        trainXs[t], mnist.train.labels,
...
# ======================================================
# ===== DEN_fd TRAINING (Functional Drift) =====
# ======================================================
...
for t in range(5):
    ...
    print("\n===== DEN_fd (Functional Drift) TASK %d =====" % (t+1))
    ...
    perfs = []
    for j in range(t+1):
        acc = den_fd_model.predict_perform(
            j+1,
            testXs[j],
            mnist.test.labels
        )
...
with tf.Session(graph=dnn.graph) as sess:
    ...
    for t in range(5):
        ...
        dnn_capacity.append(get_dnn_hidden_capacity(FLAGS.dims))

        print("DNN avg_perf:", avg)

# ======================================================
# Plot comparison (Accuracy & Capacity)
# ======================================================

plt.style.use('seaborn-v0_8-muted')
fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(22, 6))
tasks_range = range(1, 6)
tasks_labels = [f"Task {i}" for i in tasks_range]

# ======================================================
# Plot 1: Average Accuracy
# ======================================================
ax1.plot(tasks_range, den_avg_perf, marker='o', linestyle='-', linewidth=2, label="Original DEN")
ax1.plot(tasks_range, den_fd_avg_perf, marker='s', linestyle='-', linewidth=2, label="DEN (Functional Drift)")
ax1.plot(tasks_range, dnn_avg_perf, marker='^', linestyle='--', color='gray', label="Vanilla DNN")
...
# ======================================================
# Plot 2: Hidden Units Capacity
# ======================================================
ax2.plot(tasks_range, den_hidden_units, marker='o', linewidth=2, label="Original DEN")
ax2.plot(tasks_range, den_fd_hidden_units, marker='s', linewidth=2, label="DEN (Functional Drift)")
dnn_fixed_cap = get_dnn_hidden_capacity(FLAGS.dims)
ax2.plot(tasks_range, [dnn_fixed_cap]*5, linestyle='--', color='gray', label="Vanilla DNN (Fixed)")
...
# ======================================================
# Plot 3: Growth Source Comparison
# ======================================================
...
ax3.bar(x - width/2, den_expansion_history, width,
        label='DEN: Expansion', color='#1f77b4', alpha=0.6, edgecolor='white')
ax3.bar(x - width/2, den_split_history, width,
        bottom=den_expansion_history, label='DEN: Splitting', color='#aec7e8', alpha=0.6, edgecolor='white')

ax3.bar(x + width/2, fd_expansion_history, width,
        label='DEN_fd: Expansion', color='#ff7f0e', edgecolor='white')
ax3.bar(x + width/2, fd_split_history, width,
        bottom=fd_expansion_history, label='DEN_fd: Splitting', color='#ffbb78', edgecolor='white')

ax3.set_ylabel("Newly Added Hidden Units")
ax3.set_title("Growth Source: DEN vs DEN_fd", fontsize=14)
ax3.set_xticks(x)
...
for i in range(len(x)):
    ...
    ax3.text(i - width/2, total_den + 0.5, str(int(total_den)), ha='center', va='bottom', fontsize=9)
    ax3.text(i + width/2, total_fd + 0.5, str(int(total_fd)), ha='center', va='bottom', fontsize=9, fontweight='bold')
