...
class DEN(object):
    ...
    def store_memory_anchors(self, task_id, trainX, num_anchors=100):

        idx = np.random.choice(len(trainX), min(num_anchors, len(trainX)), replace=False)
        self.memory_anchors[task_id] = trainX[idx]

    def get_layer_activations(self, x_data):

        act_values = self.sess.run(self.activations_list, feed_dict={self.X: x_data})
        activations = {}
        for i, act in enumerate(act_values):
            activations['layer%d' % (i + 1)] = act
        return activations

    def detect_functional_drift(self):

        drifted_units = defaultdict(list)
        eps = 1e-8

        for old_t in range(1, self.T):
            if old_t not in self.memory_anchors:
                continue

            current_act = self.get_layer_activations(self.memory_anchors[old_t])
            baseline = self.baseline_activations[old_t]

            for layer_name, cur_val in current_act.items():
                if layer_name not in baseline:
                    continue

                base_val = baseline[layer_name]
                num_old_units = base_val.shape[1]
                relevant_cur_val = cur_val[:, :num_old_units]

                diff_sq = (relevant_cur_val - base_val) ** 2
                mse_per_unit = np.mean(diff_sq, axis=0)

                base_energy = np.mean(base_val ** 2, axis=0) + eps

                relative_drift = mse_per_unit / base_energy


                indices = np.where(relative_drift > self.spl_thr)[0]
                drifted_units[layer_name].extend(indices.tolist())

        return {k: list(set(v)) for k, v in drifted_units.items()}
    ...
    def extend_top(self, scope, ex_k=10):
        ...
        if 'layer%d' % self.n_layers == scope:
            ...
            expanded_n_units = self.expansion_layer[self.n_layers - level - 2]
    ...
    def build_model(self, task_id, prediction = False, splitting = False, expansion = None):
        ...
        if splitting:
            for i in range(1, self.n_layers):
                ...
                self.activations_list.append(bottom)
    ...
    def add_task(self, task_id, data):
        ...
        expansion_counts = [0] * (self.n_layers - 1)
        split_counts = [0] * (self.n_layers - 1)
        self.expansion_layer = [0] * (self.n_layers - 1)
        # --- [功能性漂移] 步骤 1: 记录旧任务的基准激活值 ---
        if task_id > 1:
            self.sess.run(tf.global_variables_initializer())
            temp_params = self.get_params()
            self.destroy_graph()
            self.sess = tf.Session()
            self.load_params(temp_params)

            self.set_initial_states(data_size)
            expansion_counts = [0] * (self.n_layers - 1)
            split_counts = [0] * (self.n_layers - 1)
            self.build_model(task_id - 1, prediction=True)
            self.sess.run(tf.global_variables_initializer())

            self.baseline_activations = {}
            for old_t in range(1, task_id):
                if old_t in self.memory_anchors:
                    self.baseline_activations[old_t] = self.get_layer_activations(self.memory_anchors[old_t])
        ...
        if self.T == 1:
            ...
            if c_loss < self.loss_thr:
                ...
                for i in range(self.n_layers - 1, 0, -1):
                    ...
                    added_by_ex = self.ex_k - len(useless)
                    expansion_counts[i - 1] = added_by_ex
                    self.expansion_layer[i - 1] = added_by_ex
                    print("   [*] Layer %d Expansion: +%d units" % (i, added_by_ex))

            # --- [功能性漂移] 步骤 2: 替换原本基于权重的 Split 检测逻辑 ---
            print(' [*] Split & Duplication (Functional Drift Detection)')
            ...
            drifted_units_dict = self.detect_functional_drift()

            for i in range(1, self.n_layers):
                indices = drifted_units_dict.get('layer%d' % i, [])
                if len(indices) > self.ex_k:
                    indices = indices[:self.ex_k]

                num_splits = len(indices)
                split_counts[i - 1] = num_splits

                self.expansion_layer[i - 1] += num_splits
                self.unit_indices['layer%d' % i] = np.array(indices)
                print("   [*] Layer %d Splitting: +%d units" % (i, num_splits))

            self.prev_W_split = copy.deepcopy(self.cur_W)
            ...
            print("   [*] split complete, loss: %.4f, valid_acc: %.4f" % (c_loss, val_perf))

            params = self.get_params()

        # --- [功能性漂移] 步骤 3: 任务完全结束前，存储当前任务的锚点 ---
        self.store_memory_anchors(task_id, trainX)
        ...
        print(" [*] Task: %d, Expansion Summary: %s, Split Summary: %s" % (task_id, expansion_counts, split_counts))

        return test_perf, self.avg_sparsity(task_id), (tuple(expansion_counts), tuple(split_counts))
